...
# -*- coding: utf-8 -*-
"""
MedSpeak finetune (Full FT or LoRA/QLoRA) with histogram & KG budgeting.
...
- Supports:
    * Full fine-tuning (bf16/float; sharded across visible GPUs via device_map="auto")
    * LoRA or QLoRA 4-bit (--use_4bit)
- Prints token-length histogram and top-N longest samples for prompt tuning

Tips:
- For text-only, you can skip torchvision imports:
    export TRANSFORMERS_NO_TORCHVISION=1
- Select GPUs:
    CUDA_VISIBLE_DEVICES=1,2,4,5 python utils/finetune_medspeak_ft.py ...
"""
...
def parse_args():
    ...
    # quant (QLoRA)
    ap.add_argument("--use_4bit", action="store_true", help="Enable QLoRA 4-bit (LoRA + nf4)")
    # LoRA
    ap.add_argument("--lora_r", type=int, default=16)
    ap.add_argument("--lora_alpha", type=int, default=32)
    ap.add_argument("--lora_dropout", type=float, default=0.10)
    # Full FT
    ap.add_argument("--full_finetune", action="store_true", help="Train ALL model weights (no LoRA/PEFT)")
    # KG budgets (tokens)
...
# Dataset cleaning
# =========================
def _clean_system(_: str) -> str:
    return SYS_RULES

def _clean_user(u: str) -> str:
    # Remove any trailing “Follow the system instructions …”
...
def build_model_qlora_or_fp16(args):
    """Builds a model for LoRA/QLoRA path (keeps your original behavior)."""
    if args.use_4bit:
...
def tokenize_text(tok, text: str, max_len: int):
    return tok(text, truncation=True, max_length=max_len, padding=False, return_tensors=None)
...
def main():
    ...
    if args.full_finetune and args.use_4bit:
        raise ValueError("--full_finetune is incompatible with --use_4bit (QLoRA). Choose one.")
    ...
    if args.full_finetune:
        # Full fine-tuning (no LoRA, no k-bit quant)
        model = AutoModelForCausalLM.from_pretrained(
            args.base_model,
            torch_dtype=torch.bfloat16 if torch.cuda.is_available() else torch.float32,
            device_map="auto",
        ...
        # Gradient checkpointing requires cache off
        model.config.use_cache = False
        model.gradient_checkpointing_enable()
    else:
        # LoRA / QLoRA path
        base = build_model_qlora_or_fp16(args)
        model = add_lora(base, args)
        # For PEFT + checkpointing, ensure inputs require grads
        try:
    ...
    training_args = TrainingArguments(
        ...
        tf32=True if torch.cuda.is_available() else False,  # faster matmul on Ampere+
    )
    ...
    trainer = Trainer(
        ...
        tokenizer=tok,  # FutureWarning: fine; could swap to processing_class later
    )

    # Debug: how many params train
    try:
        trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total = sum(p.numel() for p in model.parameters())
        print(f"[DEBUG] trainable params: {trainable}/{total} ({100*trainable/total:.4f}%)")
    except Exception:
        pass
    ...
    mode = "full model" if args.full_finetune else "LoRA adapter"
    print(f"[DONE] Saved {mode} + tokenizer to {args.out_dir}")
